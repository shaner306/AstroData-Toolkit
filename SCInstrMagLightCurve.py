...
# directory = r'C:\Users\jmwawrow\Documents\DRDC_Code\2021-03-10 - Calibrated\Intelsat 10-02 Post Eclipse\LIGHT\G'
# directory = r'C:\Users\jmwawrow\Documents\DRDC_Code\2021-03-10 - Calibrated\Intelsat 10-02 G Band Example'
# directory = r'C:\Users\jmwawrow\Documents\DRDC_Code\2021-03-20 - Calibrated\Intelsat 10-02 G band test'
...
none_sats = False
debugging = True
# hdistance_from_sat = int((max_distance_from_sat - 1) / 2)
temp_dir = 'tmp'
if not debugging:
    try:
        os.mkdir(temp_dir)
...
# names = np.empty(num_sats+1, dtype=object)
# names[0] = 'Time (JD)'
# names[1] = 'Intelsat 10-02'
# names[2] = "MEV 2"
# names[3] = "THOR 6"
# names[4] = 'THOR 5'
# for i in range(1, num_sats+1):
#     names[i] = f"Satellite {i}"
# times = np.ones(filecount)
# times_obj = Time(times, format='jd')
# times_fits = times_obj.to_value('fits')
# print(times_fits)
# data = np.empty((filecount, num_sats+1))
# data.fill(np.nan)
# print(times)
# print(data)
# sats_table = Table(
...
#         filenum += 1
        ...
        if change_sat_positions:
            ...
            # with fits.open(os.path.join(directory, filenames[filenum - num_nan])) as image:
                hdr = image[0].header
            ...
            if not none_sats:
                ...
                for reversing_index in range(1, num_nan+1):
                    ...
                    # with fits.open(os.path.join(directory, filenames[filenum - reversing_index])) as image:
                        hdr = image[0].header
                        imgdata = image[0].data
                    ...
                    # irafsources.pprint_all()
                    # brightest_obj.append((irafsources['xcentroid'][0], irafsources['ycentroid'][0]))
                    if plot_results != 0:
                        fig, ax = plt.subplots()
                        ax.imshow(imgdata, cmap='gray', norm=LogNorm(), interpolation='nearest')
        ...
        # psf_model = IntegratedGaussianPRF(sigma=1.6*gaussian_fwhm_to_sigma)                                       # Defime the PSF model to be used for photometry.
        psf_model.x_0.fixed = True                                                                                      # Don't change the initial 'guess' of the star x positions to be provided.
        psf_model.y_0.fixed = True                                                                                      # Don't change the initial 'guess' of the star y positions to be provided.
        # Provide the initial guesses for the x-y positions and the flux. The flux will be fit using the psf_model,
        # so that will change, but the star positions will remain the same.
...
# print(len(b_sats_table))
# print(len(g_sats_table))
# print(len(r_sats_table))
ascii.write(b_sats_table, 'CSV files/b_instr_mags_TEST.csv', format='csv')
ascii.write(g_sats_table, 'CSV files/g_instr_mags_TEST.csv', format='csv')
ascii.write(r_sats_table, 'CSV files/r_instr_mags_TEST.csv', format='csv')
ascii.write(b_uncertainty_table, 'CSV files/b_uncertainty.csv', format='csv')
ascii.write(g_uncertainty_table, 'CSV files/g_uncertainty.csv', format='csv')
ascii.write(r_uncertainty_table, 'CSV files/r_uncertainty.csv', format='csv')
# times_obj = Time(sats_table['Time (JD)'], format='jd', scale='utc')
times_obj = Time(b_sats_table['Time (JD)'], format='jd', scale='utc')
# plt.plot(sats_table['Time (JD)'], sats_table[names[1]], 'o', label=names[1])
fig, ax = plt.subplots()
for sat_num, sat in enumerate(sat_names, start=1):
    # ax.plot(times_obj.to_value('datetime'), sats_table[sat], 'o', label=sat)
...
# ascii.write(sats_table, 'InstrumentalMagnitdues_10Mar.csv', format='csv')